...
public class TpahereCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            p.sendMessage(ChatColor.RED + "Correct usage: /tpahere <player name>.");
            return true;
        }
        ...
        if (Bukkit.getPlayer(args[0]) == null || !(Bukkit.getPlayer(args[0]).isOnline())) {
            p.sendMessage(ChatColor.RED + "The player " + ChatColor.WHITE + args[0] + ChatColor.RED + " is either not online, does not exist, or is misspelled!");
            return true;
        }

        Player sentTo = Bukkit.getPlayer(args[0]);
        ProEverything.plugin.tpaPending.put(sentTo.getName(), p.getName());
        ProEverything.plugin.tpaHere.add(sentTo.getName());

        finishTpaHere(p,sentTo);
        return true;
    }

    public static void finishTpaHere(Player p, Player sentTo) {
        p.sendMessage(ChatColor.YELLOW + "You have sent a tpa here request to player " + ChatColor.WHITE + sentTo.getName() + ChatColor.YELLOW + "!");
        sentTo.sendMessage(ChatColor.YELLOW + "The player " + ChatColor.WHITE + p.getName() + ChatColor.YELLOW + " has request you to tpa to them!");
            ...
            public void run() {
                if (ProEverything.plugin.tpaPending.containsKey(p.getName())) {
                    if (!ProEverything.plugin.tpaPending.get(p.getName()).equalsIgnoreCase(sentTo.toString()))
                        return;
                    p.sendMessage(ChatColor.RED + "Tpa here request to player " + ChatColor.WHITE + sentTo.getName() + ChatColor.RED + " has expired!");
                    if (ProEverything.plugin.tpaHere.contains(sentTo.getName()))
                        ProEverything.plugin.tpaHere.remove(sentTo.getName());
                    if (sentTo.isOnline())
                        sentTo.sendMessage(ChatColor.RED + "The tpa here request from player " + ChatColor.WHITE + p.getName() + ChatColor.RED + " has expired!");
